...
int main() {
    ...
    for (size_t i = MIN_RTD_LEVEL; i < 10U; ++i) {
        ...
        oSettingsFile << i << " " << range << " " << time << " " << (bi_to_double(range) * time / SMALLEST_WHEEL) << std::endl;
#else
        oSettingsFile << i << " " << range << " " << time << " " << (range.convert_to<double>() * time / SMALLEST_WHEEL) << std::endl;
#endif
    }
    oSettingsFile.close();
