...
void kernel qimcifa_batch(global ulong* ulongArgs, global BigInteger* BigIntegerArgs, global ulong* rngSeeds, global BigInteger* outputs)
{
    ...
    bi_add(&fullMaxR, &BIG_INT_1, &fullRange);
    bi_sub_ip(&fullRange, &fullMinR);
    ...
    bi_mul(&nodeRange, &nodeIdBci, &nodeMin);
    bi_add_ip(&nodeMin, &fullMinR);
    ...
    if ((nodeId + 1U) == nodeCount) {
        ...
        bi_mul(&nodeRange, &nodeIdPlus1Bci, &nodeMax);
        bi_add_ip(&nodeMax, &fullMinR);
        bi_decrement(&nodeMax, 1);
    }

    BigInteger threadRange, threadCountBci = bi_create(nodeId);
    bi_add(&nodeMax, &BIG_INT_1, &t1);
    bi_sub_ip(&t1, &nodeMin);
    bi_div_mod(&t1, &threadCountBci, &threadRange, 0);

    BigInteger rMin, threadBci = bi_create(thread);
    bi_mul(&threadRange, &threadBci, &rMin);
    bi_add_ip(&rMin, &nodeMin);
    ...
    if ((thread + 1U) == threadCount) {
        ...
        bi_mul(&threadRange, &threadPlus1Bci, &rMax);
        bi_add_ip(&rMax, &nodeMin);
        bi_decrement(&rMax, 1);
    }

    BigInteger rRange;
    bi_copy(&rMax, &rRange);
    bi_increment(&rRange, 1);
    bi_sub_ip(&rRange, &rMin);

    BigInteger toFactorMin3;
    bi_copy(&toFactor, &toFactorMin3);
    bi_decrement(&toFactorMin3, 3);
